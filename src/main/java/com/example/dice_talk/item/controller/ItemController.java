...
public class ItemController {
    ...
    @Operation(summary = "관리자용 아이템 목록 조회", description = "아이템 목록을 조회합니다.",
            responses = {
                    @ApiResponse(responseCode = "200", description = "아이템 목록 조회 성공",
                            content = @Content(schema = @Schema(implementation = ItemDto.Response.class))),
                    @ApiResponse(responseCode = "403", description = "조회 권한 없음",
    ...
    @GetMapping("/admin")
    public ResponseEntity<MultiResponseDto<ItemDto.Response>> getItemsAdmin(@Parameter(name = "page", description = "조회할 페이지 번호 (1부터 시작)", example = "1")
                                                                            @Positive @RequestParam(defaultValue = "1") int page,
                                                                            @Parameter(name = "size", description = "한 페이지당 항목 수", example = "10")
                                                                            @Positive @RequestParam(defaultValue = "10") int size,
                                                                            @Parameter(name = "sort", description = "조회 시 기준컬럼", example = "itemId/dicePrice,asc/desc")
                                                                            @RequestParam(defaultValue = "dicePrice,asc") String sort) {
        AuthorizationUtils.isAdmin();
        Page<Item> itemPage = itemService.findItemsAdmin(page, size, sort);
        List<Item> items = itemPage.getContent();
        return new ResponseEntity<>(new MultiResponseDto<>(mapper.itemsToItemResponses(items), itemPage), HttpStatus.OK);
    }
